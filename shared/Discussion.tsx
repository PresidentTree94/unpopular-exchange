...
export default function Discussion() {
  ...
  const loadComments = async (takeId: number) => {
    const { data: comments } = await supabase.from("comments").select("*").eq("take_id", takeId);
    ...
    <>
      ...
      <div className={`box p-8 ${brdrClr} @container`}>
        ...
        <form className="flex flex-col @3xs:flex-row gap-2 mt-4 mb-6" onSubmit={handleSubmit}>
          ...
          <button disabled={content.trim() === ""} className={`${btnClr} text-white rounded-lg flex items-center justify-center h-11 aspect-square disabled:opacity-75`}><FontAwesomeIcon icon={faPaperPlane} className="!w-auto !h-5" /></button>
        </form>
